...
export function computeStats(snap) {
  ...
  // ---- 轮次起止配对（按 turn 号）----
  const turnRanges = new Map(); // turn → { start, end, firstText, lastText }
  for (const evt of events) {
    if (evt.turn === undefined || evt.turn === null) continue;
    let r = turnRanges.get(evt.turn);
    if (!r) { r = { start: null, end: null, firstText: null, lastText: null }; turnRanges.set(evt.turn, r); }
    if (evt.kind === EVENT_TYPES.TURN_START) { r.start = evt.time; r.firstText = evt.label; }
    else if (evt.kind === EVENT_TYPES.TURN_END) { r.end = evt.time; r.lastText = evt.endLabel; }
  }
  const turnDurations = [];
  let totalTurnMs = 0;
